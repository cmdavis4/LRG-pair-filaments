...
def get_chunk(chunksize, chunknum):
    ...
    cmd = 'select id, ra_mid, dec_mid, z, 15000 / (kpc_per_arcsec * 60), .0125, .065 from LRG_pairs where R_pair between 6 and 10 order by id limit %d, %d;' % ((chunknum-1) * chunksize, chunksize)
    print(cmd)
    data = cursor.get_data(cmd)
    chunkdata = {}
    for a in zip(data,['id', 'ra_gal','dec_gal','zgal', 'theta', 'deltaz_spec', 'deltaz_photo'], [int, float, float, float, float, float, float]):
...
"""
    ...
    cmd +="""dbo.fGetNearbyObjEq(@ra,@dec,@theta) as N 
WHERE 
p.objid=N.objid
and z.objid=p.objid
"""
